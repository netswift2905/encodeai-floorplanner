...
const oneCreditPriceId = process.env.STRIPE_PRICE_ID_ONE_CREDIT!
const threeCreditsPriceId = process.env.STRIPE_PRICE_ID_THREE_CREDITS!
const fiveCreditsPriceId = process.env.STRIPE_PRICE_ID_FIVE_CREDITS!
    ...
    case 'checkout.session.completed':
      ...
      if (existingCredits) {
        ...
        if (error) {
          ...
          return NextResponse.json(
            {
              message: `Error updating credits: ${JSON.stringify(error)}\n ${data}`,
            },
            {
              status: 400,
            }
