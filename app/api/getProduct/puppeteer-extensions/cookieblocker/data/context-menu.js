...
chrome.runtime.onMessage.addListener(function (request, info) {
  if (request == 'update_whitelist') updateWhitelist()
})

function isWhitelisted(tab) {
  if (typeof whitelisted_domains[tab.hostname] != 'undefined') return true
...
function toggleWhitelist(tab) {
  if (tab.url.indexOf('http') != 0 || !tab_list[tab.id]) return
  ...
  } else whitelisted_domains[tab_list[tab.id].hostname] = true
...
function onUpdatedListener(tabId, changeInfo, tab) {
  if (changeInfo.status) tab_list[tab.id] = getPreparedTab(tab)
}

function onRemovedListener(tabId) {
  if (tab_list[tabId]) delete tab_list[tabId]
}
...
function recreateTabList() {
  ...
  chrome.tabs.query({}, function (results) {
    ...
    for (var i in tab_list) doTheMagic(tab_list[i].id)
  })
}
    ...
    for (var group in block_urls.common_groups) {
      if (d.url.indexOf(group) > -1) {
        ...
        for (var i in group_filters) {
          if (
            (group_filters[i].q && d.url.indexOf(group_filters[i].r) > -1) ||
            ...
            if (group_filters[i].e && tab_list[d.tabId].host_levels.length > 0)
              for (var level in tab_list[d.tabId].host_levels)
                for (var exception in group_filters[i].e)
                  if (
                    group_filters[i].e[exception] ==
                    tab_list[d.tabId].host_levels[level]
                  )
                    return { cancel: false }
    ...
    if (d.tabId > -1 && tab_list[d.tabId].host_levels.length > 0) {
      for (var level in tab_list[d.tabId].host_levels) {
        if (block_urls.specific[tab_list[d.tabId].host_levels[level]]) {
          ...
          for (var i in rules)
            if (d.url.indexOf(rules[i]) > -1) return { cancel: true }
        }
      }
    }
  }
...
function reportWebsite(info, tab) {
  ...
  chrome.tabs.create({
    url:
      'https://www.i-dont-care-about-cookies.eu/report/' +
      chrome.runtime.getManifest().version +
      '/' +
      encodeURIComponent(encodeURIComponent(tab.url)),
  })
}
...
function activateDomain(hostname, tabId, frameId) {
  if (!cached_rules[hostname]) cached_rules[hostname] = rules[hostname] || {}
  ...
  if (typeof r.j != 'undefined') {
    chrome.tabs.executeScript(tabId, {
      file: 'data/js/' + (r.j > 0 ? 'common' + r.j : hostname) + '.js',
      frameId: frameId,
      matchAboutBlank: true,
      runAt: 'document_end',
    })
...
function doTheMagic(tabId, frameId, anotherTry) {
  if (!tab_list[tabId] || tab_list[tabId].url.indexOf('http') != 0) return

  if (tab_list[tabId].whitelisted) return

  // Common CSS rules
  chrome.tabs.insertCSS(
    tabId,
    {
      file: 'data/css/common.css',
      frameId: frameId || 0,
      matchAboutBlank: true,
      runAt: 'document_start',
    },
    function () {
      ...
      if (activateDomain(tab_list[tabId].hostname, tabId, frameId || 0)) return

      for (var level in tab_list[tabId].host_levels)
        if (
          activateDomain(
            tab_list[tabId].host_levels[level],
            tabId,
            frameId || 0
          )
        )
...
chrome.webRequest.onResponseStarted.addListener(
  function (tab) {
    if (tab.frameId > 0) doTheMagic(tab.tabId, tab.frameId)
  },
      ...
      if (request.command == 'get_active_tab') {
        ...
        chrome.tabs.create({
          url:
            ...
            chrome.runtime.getManifest().version +
            '/' +
            encodeURIComponent(encodeURIComponent(tab_list[request.tabId].url)),
        })
