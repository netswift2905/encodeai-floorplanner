...
var main_interval = setInterval(function () {
  ...
  if (!html || /idc8_350/.test(html.className)) return
  ...
  var counter = 0,
    host_parts = document.location.hostname.split('.'),
      ...
      function () {
        ...
        if (host_parts[0] == 'consent') {
          if (document.location.pathname == '/m') {
            if (host_parts[1] == 'youtube')
              element = _sl(
                'div + form[action*="//consent."][action$="/save"] button, .basebutton + form[action*="//consent."][action*="/save"] .button'
              )
            else
              element = _sl(
                'form[action*="//consent."][action$="/save"]:first-of-type:not(:only-of-type) button'
              )
          ...
          // Mobile only: 1. google.co.uk (or in FF Nightly, on google.com search results) 2. YT only, a copy or the desktop selector
            ...
            element = _sl(
              '.saveButtonContainerNarrowScreen > form:last-child .button, .basebutton + form[action*="//consent."][action*="/save"] .button'
            )
        ...
        // https://www.google.com/finance/
          ...
          if (
            container &&
            _sl(
              'a[href*="policies.google.com/technologies/cookies"]',
              container
            )
          ) {
